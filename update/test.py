...
class QRCodeScannerApp:
    def __init__(self, root):
        ...
        # open json file by notepad
        self.open_button = Button(self.button_frame, text="Open settings .json", command=self.open_json, font=("Segoe UI", 12), bg="#FF5722", fg="white", padx=10, pady=5)
        self.open_button.grid(row=1, column=4, padx=10, pady=5, sticky=tk.W+tk.E)
    ...
    def display_image(self, image_path):
        """
        เปิดและแสดงรูปภาพใน GUI
        """
        # โหลดรูปภาพจากเส้นทางที่กำหนด
        image = Image.open(image_path)
        image = image.resize((self.video_label.winfo_width(), self.video_label.winfo_height()))
        tk_image = ImageTk.PhotoImage(image)

        # แสดงรูปภาพใน หน้าต่างใหม่ และ ขนาดของรูปภาพจะเป็นขนาดเดียวกับรูปภาพ
    ...
    def scan(self):
        ...
        if ret:
            ...
            for obj in decoded_objects:
                ...
                # Check if data matches any item in settings
                if data in settings:
                    file_type = settings[data].get("file_type")
                    file_path = settings[data].get("path")

                    # Open the appropriate file type
                    if file_type == "image" and os.path.exists(file_path):
                        print(f"Displaying image: {file_path}")
                        self.display_image(file_path)  # Method to display image
                    elif file_type == "video" and os.path.exists(file_path):
                        print(f"Playing video: {file_path}")
                        self.play_video(file_path)  # Method to play video
                    else:
                        print("File not found or invalid file type.")
                else:
                    print("No matching settings found for QR code data.")
